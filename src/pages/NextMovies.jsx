...
const getNextPage = async (url) => {
  ...
  setNextMovies([...nextMovies, data.results].flat());

};

const GetNextMovies = async (url) => {
  const res = await fetch(url);
  const data = await res.json();
  setNextMovies([...nextMovies, data.results].flat());
};
...
const NextPage = () => {
    ...
    const urlnextpage = `https://api.themoviedb.org/3/movie/upcoming?api_key=4888028033e53f9aa150a7b1fd5bf7ca&language=pt-BR&page=${page}`
...
useEffect(() => {
  const urlPopularMovies = `https://api.themoviedb.org/3/movie/upcoming?api_key=4888028033e53f9aa150a7b1fd5bf7ca&language=pt-BR&page=${page}`
  GetNextMovies(urlPopularMovies)
  if (page < 86) {
    setPage(page + 1)
  }
